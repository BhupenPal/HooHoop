...
Router.get("/", async (req, res) => {
  ...
  const popularCars = await carModel.find({adActive: "Active"}).limit(10).sort({views: -1}).exec();
  const hatchCars = await carModel.find({BodyType: "Hatchback", adActive: "Active"}).limit(10).exec();
  const sedanCars = await carModel.find({BodyType: "Sedan", adActive: "Active"}).limit(10).exec();
  const PriceFive = await carModel.find({Price: {$lt: 5000}, adActive: "Active"}).limit(10).exec();
  const PriceTen = await carModel.find({Price: {$gt:5000 ,$lt: 10000}, adActive: "Active"}).limit(10).exec();
  const PriceGtTen = await carModel.find({Price: {$gt: 10000}, adActive: "Active"}).limit(10).exec();
